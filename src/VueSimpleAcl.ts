 ...
 * @return void
 */
function addAclAbility(ability: string, callback: AnyFunction): void {
 ...
 * @return void
 */
function setRule(abilities: Ability, callback: AnyFunction): void {
 ...
 * @return boolean
 */
function evaluateAbilityCallback(abilityCallback: any, ability: Ability, args?: AbilityArgs): boolean {
  try {
 ...
 * @return boolean
 */
function checkAclAbilities({ abilities, args, any = false }: AbilitiesEvaluationProps): boolean {
 ...
 * @return boolean
 */
function prepareAcl({ abilities, args, any = false }: AbilitiesEvaluationProps): boolean {
 ...
 * @return {boolean}
 */
function helperArgsToPrepareAcl({ abilities, args, any = false }: AbilitiesEvaluationProps): boolean {
  if (abilities && typeof abilities === 'string') {
    return prepareAcl({
      abilities,
      args,
 ...
 * @return boolean
 */
function canHelperHandler(abilities: Ability, args?: AbilityArgs): boolean {
  return helperArgsToPrepareAcl({ abilities, args, any: false })
}
 ...
 * @return boolean
 */
function notCanHelperHandler(abilities: Ability, args?: AbilityArgs): boolean {
  return !helperArgsToPrepareAcl({ abilities, args, any: false })
}
 ...
 * @return boolean
 */
function anyCanHelperHandler(abilities: Ability, args?: AbilityArgs): boolean {
  return helperArgsToPrepareAcl({ abilities, args, any: true })
}
 ...
 * @returns boolean
 */
function hasAsyncUser<U = User>(user: U | AsyncUser<U>): user is AsyncUser<U> {
...
export function installPlugin<U = User>(app: any, options?: PluginOption<U>) {
  ...
  const directiveHandler = (el: any, binding: any) => {
    ...
    const anyModifier = !!(aclModifiers.any)
    const notModifier = !!(aclModifiers.not)
    const readonlyModifier = !!(aclModifiers.readonly)
    const disableModifier = !!((aclModifiers.disable || aclModifiers.disabled))
    const hideModifier = !!((aclModifiers.hide || aclModifiers.hidden))
 ...
 * @return object
 */
export function createAcl<U = User>(userDefinedOptions: PluginOption<U>): any {
  return {
 ...
 * @return void
 */
export function defineAclRules<U = User>(aclRulesCallback: (setter: RuleSetter<U>) => void): void {
